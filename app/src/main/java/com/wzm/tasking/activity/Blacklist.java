...
public class Blacklist extends AppCompatActivity {
    ...
    private void initView() {
        ...
        mToolbar = (Toolbar) findViewById(R.id.toolbar_blacklist);
        mTabLayout = (TabLayout) findViewById(R.id.tab_blacklist);
        mViewPager = (ViewPager) findViewById(R.id.vp_blacklist);
    ...
    private void initListener() {
        ...
        mList_1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                i.setClass(Blacklist.this, GetUserInfo.class);
                startActivity(i);
                finish();
            }
        });
        mList_2.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                i.putExtra("id", ID);
                i.setClass(Blacklist.this, GetGroupInfo.class);
                startActivity(i);
                finish();
            }
        });
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        menu.add(1, 1, 1, getResources().getString(R.string.out_blacklist));
    }
    ...
    private void getBlockedGroupsList() {
        new Thread() {
            @Override
            public void run() {
                mData2.clear();
                JMessageClient.getBlockedGroupsList(new GetGroupInfoListCallback() {
                    @Override
                    public void gotResult(int i, String s, List<GroupInfo> list) {
                        if (i == 0) {
                            if (list == null) return;
    ...
    private void del_user(final int p) {
        try {
            ...
            mNames.add(mData1.get(p).getUserName());
            JMessageClient.delUsersFromBlacklist(mNames, null,
                    new BasicCallback() {
                        @Override
                        public void gotResult(int i, String s) {
                            if (i == 0) {
                                if (mData1.get(p).isFriend()) {
                                    for (UserInfo userInfo : F_2.mData_f) {
                                        if (userInfo.getUserName().
                                                equals(mData1.get(p).getUserName())) {
                                            userInfo.setNoteText("0");
                                            break;
                                        }
                                    }
                                }
    ...
    private void del_group(final int p) {
        try {
            ...
            Info.setBlockGroupMessage(0, new GetBlacklistCallback() {
                @Override
                public void gotResult(int i, String s, List<UserInfo> list) {
                    if (i == 0) {
                        mData2.remove(p);
